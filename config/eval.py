...
def evaluate_batch_insts(batch_insts: List[Instance],
    ...
    for idx in range(len(batch_pred_ids)):
        ...
        named_entity = []
        for i in range(len(output)):
            if (output[i].startswith('B-') or output[i] == 'O') and len(named_entity) > 0:
                output_spans.add(tuple(named_entity))
                named_entity = []
            if output[i].startswith("B-"):
                named_entity.append(i)
            if output[i].startswith("I-") and len(named_entity) > 0:
                named_entity.append(i)
        if len(named_entity) > 0:
            named_entity.append(tuple(named_entity))
        
        predict_spans = set()
        named_entity = []
        for i in range(len(prediction)):
            if (prediction[i].startswith('B-') or prediction[i] == 'O') and len(named_entity) > 0:
                output_spans.add(tuple(named_entity))
                named_entity = []
            if prediction[i].startswith("B-"):
                named_entity.append(i)
            if prediction[i].startswith("I-") and len(named_entity) > 0:
                named_entity.append(i)
        if len(named_entity) > 0:
            named_entity.append(tuple(named_entity))
